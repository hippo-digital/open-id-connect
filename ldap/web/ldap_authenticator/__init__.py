...
class ldap_authenticator:
    def __init__(self, server, port, service_bind_dn, service_password, search_base_dn_list, claim_attributes):
        self.log = logging.getLogger('ldap.authn')

        self.service_bind_dn = service_bind_dn
        self.service_password = service_password
        self.search_base_dn_list = search_base_dn_list
    ...
    def verify_user(self, transaction_id, username, password):
        ...
        for search_base_dn in self.search_base_dn_list:
            conn.search(search_base_dn, '(&(objectclass=person)(uid=%s))' % username, attributes=ldap3.ALL_ATTRIBUTES)
